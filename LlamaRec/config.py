...
def set_template(args):
    ...
    if 'llm' in args.model_code:
        batch = 12 if args.dataset_code == 'ml-100k' else 8
    ...
    if args.dataset_code == 'ml-100k':
        args.llm_relations_class = args.llm_relations_class_movielens
        args.llm_max_text_len_relation = args.llm_max_text_len_relation_movielens
        args.llm_max_title_len = args.llm_max_title_len_movielens
    elif args.dataset_code == 'beauty':
        args.llm_relations_class = args.llm_relations_class_beauty
        args.llm_max_text_len_relation = args.llm_max_text_len_relation_beauty
        args.llm_max_title_len = args.llm_max_title_len_beauty
...
parser.add_argument('--llm_max_title_len', type=int, default=0)
parser.add_argument('--llm_max_title_len_movielens', type=int, default=32)
parser.add_argument('--llm_max_title_len_beauty', type=int, default=10)
parser.add_argument('--llm_max_text_len', type=int, default=1536)
parser.add_argument('--llm_max_text_len_relation', type=int, default=0)
parser.add_argument('--llm_max_text_len_relation_movielens', type=int, default=2286)
parser.add_argument('--llm_max_text_len_relation_beauty', type=int, default=2536)
...
parser.add_argument('--llm_train_with_relation', type=bool, default=True) # training with relations
parser.add_argument('--llm_system_template_relation', type=str,  # instruction for training with relations
    default="Given user history in chronological order and the relations, recommend an item from the candidate pool with its index letter.")
parser.add_argument('--llm_input_template_relation', type=str, \
    default='User history: {}; \n Relations: {}; \n Candidate pool: {}')
parser.add_argument('--llm_train_with_relation_score', type=bool, default=True) # training with user preferences
parser.add_argument('--llm_user_embedding_dim', type=int, default=512) # user embedding dim, 128 for movielens, 512 for beauty
parser.add_argument('--llm_relations_class', type=list, default=None)
parser.add_argument('--llm_relations_class_movielens', type=list, default=['GENRE_IS', 'RELEASED_YEAR_IS', 'DIRECTED_BY', 'HAS_ACTOR'])
parser.add_argument('--llm_relations_class_beauty', type=list, default=['BRAND_IS', 'CATEGORY_IS', 'ALSO_BOUGHT', 'ALSO_VIEWED', 'BOUGHT_TOGETHER'])
...
parser.add_argument('--llm_only_eval', type=bool, default=True)
parser.add_argument('--llm_continue_training', type=bool, default=False)
...
parser.add_argument('--lora_max_steps', type=int, default=-1) # set to -1 for using epochs instead of steps
...
# Neo4j
################
parser.add_argument('--neo4j_uri', type=str, default="YOUR_URI")
parser.add_argument('--neo4j_user_name', type=str, default="neo4j")
parser.add_argument('--neo4j_password', type=str, default="YOUR_PASSWORD")
