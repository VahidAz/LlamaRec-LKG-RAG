...
def seq_to_token_ids(args, seq, candidates, label, text_dict, tokenizer, prompter, eval=False,
    ...
    seq_t = ' \n '.join(['(' + str(idx + 1) + ') ' + truncate_title(item) 
                       for idx, item in enumerate(seq)])
    can_t = ' \n '.join(['(' + chr(ord('A') + idx) + ') ' + truncate_title(item)
                       for idx, item in enumerate(candidates)])
    output = chr(ord('A') + candidates.index(label))  # ranking only
    
    if not args.llm_train_with_relation: #or len(relations) <= 5:
        data_point = {}
...
# This one was the last version which is used for movielense
